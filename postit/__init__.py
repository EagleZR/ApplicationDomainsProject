...
def user(user_id):
    ...
    if request.method == 'GET':
        if user_id == 'all':
            ...
            if str(requester_user_id) == str(user_id) or user_type == "admin":
                response = jsonify(db.get_user(user_id))
                response.status_code = 200
                return response
            else:
                raise get_error_response(403, "You are not authorized to view this user's information")
        ...
        if data is not None:
            ...
            if user_id == requester_user_id:
                ...
                if category == 'user_type':
                    ...
                    if db.set_user_data(user_id, category, value):
                        response = jsonify({"message": "The data was successfully set"})
                        response.status_code = 200
                        return response
                    else:
                        response = jsonify({"message": "The data could not be set"})
